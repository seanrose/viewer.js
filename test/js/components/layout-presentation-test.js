...
QUnit.cases([
    ...
    dom.html(container, VIEWER_HTML_TEMPLATE);

    var docEl = dom.find('.' + CSS_CLASS_DOC, container);

    dom.appendTo(docEl, PAGE_HTML_TEMPLATE);
    dom.appendTo(docEl, PAGE_HTML_TEMPLATE);
    dom.appendTo(docEl, PAGE_HTML_TEMPLATE);

    var pageEls = dom.findAll('.' + CSS_CLASS_PAGE, docEl);

    if (params.currentPage) {
        dom.addClass(pageEls[params.currentPage - 1], CSS_CLASS_CURRENT_PAGE);
    }
    if (params.precedingPage) {
        dom.addClass(pageEls[params.precedingPage - 1], CSS_CLASS_PRECEDING_PAGE);
    }
    ...
    if (params.expectedCurrent) {
        ok(dom.hasClass(pageEls[params.expectedCurrent - 1], CSS_CLASS_CURRENT_PAGE), 'should have current page class');
    }
    if (params.expectedPreceding) {
        ok(dom.hasClass(pageEls[params.expectedPreceding - 1], CSS_CLASS_PRECEDING_PAGE), 'should have preceding page class');
        equal(dom.findAll('.' + CSS_CLASS_PRECEDING_PAGE, docEl).length, 1, 'should only be one preceding page');
    } else {
        equal(dom.findAll('.' + CSS_CLASS_PRECEDING_PAGE, docEl).length, 0, 'should not be a preceding page');
    }
    equal(dom.findAll('.' + CSS_CLASS_CURRENT_PAGE, docEl).length, 1, 'should only be one current page');
});
